...
def process_video_auto(video_file, progress=gr.Progress()):
    ...
    global GLOBAL_LOG
    GLOBAL_LOG = LogCollector()  # Reset log
    ...
    try:
        if video_file is None:
            return (
                "⚠️ Please upload a video file",  # status
                gr.update(visible=False),  # detected_info
                gr.update(visible=False),  # face_gallery
                gr.update(visible=False),  # output_video
                None,  # video_dict
                gr.update(visible=False),  # annotated_frame
                ""  # processing_log
            )
        ...
        temp_dir = None
        try:
            # Create temporary directory in HF Space temp location
            temp_dir = tempfile.mkdtemp(dir=TEMP_DIR)
            print(f"Created temporary directory: {temp_dir}")
            
            # Detect all faces in the video
            progress(0.1, desc="Detecting speakers in video...")
            current_status = "🔍 Detecting speakers in video..."
            ...
            if error_msg:
                ...
                return (
                    ...
                    gr.update(visible=False),  # output_video
                    None,  # video_dict
                    gr.update(value=annotated_frame if annotated_frame is not None else None, visible=True),  # annotated_frame
                    GLOBAL_LOG.get_text()  # processing_log
                )
            
            if num_speakers == 0:
                print("No speakers detected in the video.")
                return (
                    "❌ No speakers detected in the video. Please ensure faces are visible in the first frame.",
                    gr.update(visible=False),
                    gr.update(visible=False),
                    gr.update(visible=False),
                    None,
                    gr.update(value=annotated_frame if annotated_frame is not None else None, visible=True),
                    GLOBAL_LOG.get_text()
                )
            ...
            # Process video with detected number of speakers
            progress(0.3, desc=f"Separating {num_speakers} speakers...")
            current_status = f"🎬 Separating {num_speakers} speakers..."
            ...
            try:
                ...
                # Poll status updates from the GPU callback to drive the UI progress bar.
                ...
                video_dict = {i: output_files[i] for i in range(num_speakers)}
                video_dict['temp_dir'] = temp_dir
                
                progress(1.0, desc="Complete!")
                
                return (
                    f"✅ Successfully separated {num_speakers} speakers! Click on any face below to view their video.",
                    gr.update(value=detected_info, visible=True),
                    gr.update(value=face_gallery_images, visible=True),
                    gr.update(value=output_files[0], visible=True),
                    video_dict,
                    gr.update(value=annotated_frame, visible=True),
                    GLOBAL_LOG.get_text()
                )
